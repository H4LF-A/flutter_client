...
class Native {
  ...
  /// Lists output devices currently available from LiveKit's own Android
  /// audio session manager - the one actually in control of routing, unlike
  /// flutter_webrtc's own MediaDevices.enumerateDevices()/AudioSwitchManager
  /// output enumeration, which this plugin disables on init and which
  /// therefore always reports an empty output device list on Android.
  @internal
  static Future<List<Map<String, dynamic>>> getAndroidAvailableOutputDevices() async {
    try {
      final response = await channel.invokeMethod<dynamic>('getAvailableOutputDevices');
      if (response is List) {
        return response
            .whereType<Map>()
      ...
      logger.warning('getAvailableOutputDevices did throw $error');
    }
    return const <Map<String, dynamic>>[];
  }

  /// Selects a specific output device by the id returned from
  /// [getAndroidAvailableOutputDevices]. A null id clears the explicit
  /// selection, letting routing fall back to the preferred-device list.
  @internal
  static Future<void> selectAndroidOutputDevice(String? deviceId) async {
    try {
      await channel.invokeMethod<void>(
        'selectOutputDevice',
      ...
      logger.warning('selectOutputDevice did throw $error');
    }
  }
