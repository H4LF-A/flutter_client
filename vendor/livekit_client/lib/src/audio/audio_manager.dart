...
class AudioManager {
  ...
  /// Lists output devices (speaker, earpiece, wired headset, Bluetooth)
  /// currently available from LiveKit's own Android audio session manager -
  /// the one actually in control of routing. Android only; returns an empty
  /// list on other platforms.
  ///
  /// Deliberately bypasses flutter_webrtc's own
  /// MediaDevices.enumerateDevices() output-device enumeration: this plugin
  /// disables flutter_webrtc's competing AudioSwitchManager on init, which
  /// makes that enumeration always report an empty list on Android.
  Future<List<Map<String, dynamic>>> getAndroidOutputDevices() async {
    if (!lkPlatformIs(PlatformType.android)) {
      return const <Map<String, dynamic>>[];
    }
    return Native.getAndroidAvailableOutputDevices();
  }

  /// Selects a specific output device by the id returned from
  /// [getAndroidOutputDevices]. Pass null to clear the explicit selection and
  /// fall back to the preferred-device list (headset/Bluetooth over speaker,
  /// or the reverse - see [setSpeakerOutputPreferred]). Android only.
  Future<void> selectAndroidOutputDevice(String? deviceId) async {
    if (!lkPlatformIs(PlatformType.android)) {
      return;
    }
