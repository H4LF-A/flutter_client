...
internal class LKAudioSwitchManager(private val context: Context) {
  ...
  /**
   * Selects a specific output device by the type name produced by
   * [outputDeviceTypeName] (e.g. from a prior [getAvailableOutputDevices]
   * call). A null or unmatched id clears the explicit selection, letting
   * routing fall back to the preferred-device list.
   */
  @Synchronized
  fun selectOutputDevice(deviceId: String?) {
    val switch = audioSwitch ?: return
    handler.post {
      val target = deviceId?.let { id ->
        switch.availableAudioDevices.firstOrNull { outputDeviceTypeName(it) == id }
      }
...
internal fun outputDeviceTypeName(device: AudioDevice): String = when (device) {
