...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
  ...
  @Nullable ConstraintsMap androidAudioConfiguration, Severity logSeverity, @Nullable Integer audioSampleRate, @Nullable Integer audioOutputSampleRate) {
    ...
    if(bypassVoiceProcessing) {
      ...
      // Hardware AEC/NS used to be enabled unconditionally on API 29+ here.
      // WebRTC's own software APM (which the app's per-call noise-suppression
      // tier actually configures via MediaConstraints) is skipped whenever
      // hardware processing is active, to avoid double-processing - so the
      // hardware-on default silently defeated every tier the app offered
      // except the ones that also happen to match hardware's fixed behavior.
